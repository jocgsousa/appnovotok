    ...
    try {
        ...
        if (
            ...
            echo json_encode(array(
                "success" => false,
                "message" => "Dados incompletos. ID, RID, código do departamento e descrição são obrigatórios."
            ));
            exit;
        }
        ...
        $check_query = "SELECT COUNT(*) as count FROM departamentos WHERE codpto = :codpto AND id != :id";
        $check_stmt = $db->prepare($check_query);
        ...
        if ($row['count'] > 0) {
            ...
            echo json_encode(array(
                "success" => false,
                "message" => "Já existe outro departamento com este código."
            ));
            exit;
        }
        ...
        $query = "UPDATE departamentos SET 
                  rid = :rid, 
                  atualizainvgeral = :atualizainvgeral, 
                  codpto = :codpto, 
                  descricao = :descricao, 
