...
try {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM vendas_totais WHERE id = :id");
    ...
    $sql = "UPDATE vendas_totais SET ";
    $params = ['id' => $data['id']];
    
    // Log dos valores recebidos para diagnóstico
    error_log("===== VALORES RECEBIDOS EM atualizar_venda_total.php =====");
    ...
    foreach ($campos_atualizaveis as $campo) {
        if (isset($data[$campo])) {
            ...
            if (in_array($campo, ['total_qtd_pedidos', 'total_media_itens', 'total_ticket_medio', 'total_vlcustofin', 'total_via', 'total_vlvendadodia', 'total_vldevolucao', 'total_valor'])) {
                $params[$campo] = (float) $data[$campo];
                error_log("$campo: " . $data[$campo] . " -> " . $params[$campo] . " (convertido para float)");
            } else if ($campo === 'total_qtcliente') {
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Venda total atualizada com sucesso',
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Erro ao atualizar venda total: ' . $e->getMessage()
    ]);
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode([
