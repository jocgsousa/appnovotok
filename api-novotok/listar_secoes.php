...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        ...
        // Filtrar por código de seção se fornecido
        if (isset($_GET['codsec']) && !empty($_GET['codsec'])) {
            $query .= " AND s.codsec = :codsec";
            $params[':codsec'] = $_GET['codsec'];
        }

        // Filtrar por código de departamento se fornecido
        if (isset($_GET['codpto']) && !empty($_GET['codpto'])) {
            $query .= " AND s.codpto = :codpto";
        ...
        // Ordenação
        $query .= " ORDER BY s.codpto ASC, s.codsec ASC";
        ...
        echo json_encode(array(
            "success" => false,
            "message" => "Erro ao listar seções: " . $e->getMessage()
        ));
    }
} else {
