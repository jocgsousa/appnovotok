...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Log para depuração
    error_log("Tentando autorizar aparelho - ID: " . ($id ?? 'não fornecido') . ", Código: " . ($codaparelho ?? 'não fornecido'));
    ...
    if ($id !== null) {
        ...
        // Atualiza o atributo autorized do aparelho para 1 usando o ID
        $sql = "UPDATE aparelhos SET autorized = 1 WHERE id = :id";
    ...
    // Se apenas o codaparelho foi fornecido
        ...
        // Atualiza o atributo autorized do aparelho para 1 usando o codaparelho
        $sql = "UPDATE aparelhos SET autorized = 1 WHERE codaparelho = :codaparelho";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':codaparelho', $codaparelho);
    }

    if ($stmt->execute()) {
        echo json_encode(['success' => true, 'message' => 'Aparelho autorizado com sucesso.']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Erro ao autorizar aparelho: ' . $stmt->errorInfo()[2]]);
    }
} else {
