...
try {
    ...
    $sql = "INSERT INTO bijou_filial_config (filial_id, departamentos, secoes, ativo)
    ...
    foreach ($payload['items'] as $item) {
        ...
        // bindParam não aceita expressões por referência; usar bindValue com variáveis
    ...
    $pdo->commit();
    echo json_encode(['success' => true, 'message' => 'Configurações salvas', 'data' => ['salvos' => $salvos]]);
} catch (Exception $e) {
    if ($pdo && $pdo->inTransaction()) {
        $pdo->rollBack();
    }
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Erro ao salvar configuração', 'error' => $e->getMessage()]);
}
?>