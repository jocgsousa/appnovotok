...
try {
    ...
    if (!isset($_GET['documento']) || empty($_GET['documento'])) {
        ...
        echo json_encode(['success' => false, 'message' => 'CPF/CNPJ não fornecido']);
        exit;
    }

    // Limpar o documento (remover caracteres especiais)
    $documento = preg_replace('/[^0-9]/', '', $_GET['documento']);
    ...
    $sql = "SELECT c.*, a.ramo as ramo_nome, pc.nomecidade as cidade_nome, pc.uf 
            ...
            WHERE c.person_identification_number = :documento 
            LIMIT 1";
    ...
    error_log("Erro PDO ao buscar cliente por CPF/CNPJ: " . $e->getMessage());
