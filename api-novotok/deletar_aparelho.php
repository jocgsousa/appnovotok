...
// Verifica se a requisição é do tipo DELETE ou POST
if ($_SERVER['REQUEST_METHOD'] === 'DELETE' || $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Log para depuração
    error_log("Tentando deletar aparelho - ID: " . ($id ?? 'não fornecido') . ", Código: " . ($codaparelho ?? 'não fornecido'));
    ...
    if ($id !== null) {
        ...
        // Exclui o aparelho da base de dados usando o ID
    ...
    // Se apenas o codaparelho foi fornecido
        ...
        // Exclui o aparelho da base de dados usando o codaparelho
        $sql = "DELETE FROM aparelhos WHERE codaparelho = :codaparelho";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':codaparelho', $codaparelho);
    }

    if ($stmt->execute()) {
        echo json_encode(['success' => true, 'message' => 'Aparelho excluído com sucesso.']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Erro ao excluir aparelho: ' . $stmt->errorInfo()[2]]);
    }
} else {
    echo json_encode(['success' => false, 'message' => 'Método de requisição inválido. Use DELETE ou POST.']);
}
