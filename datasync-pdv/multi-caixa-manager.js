...
class MultiCaixaManager {
    constructor(externalConfig = null) {
        ...
        // Configurar handlers para capturar eventos que possam parar a aplicação
        this.setupGlobalErrorHandlers();
        
        // Inicializar monitor do sistema
        this.systemMonitor = new SystemMonitor();
        this.systemMonitor.startMonitoring(30000); // Monitor a cada 30 segundos
    }
    
    // Configurar handlers globais para capturar eventos que possam parar a aplicação
    setupGlobalErrorHandlers() {
        // Capturar erros não tratados
        process.on('uncaughtException', (error) => {
            this.logStateChange('uncaughtException', `Erro não tratado: ${error.message}`, error.stack);
            console.error('🚨 [CRITICAL] Erro não tratado:', error);
        });
        
        // Capturar promises rejeitadas não tratadas
        process.on('unhandledRejection', (reason, promise) => {
            this.logStateChange('unhandledRejection', `Promise rejeitada não tratada: ${reason}`, new Error().stack);
            console.error('🚨 [CRITICAL] Promise rejeitada não tratada:', reason);
        });
        
        // Capturar sinais de encerramento
        process.on('SIGTERM', () => {
            this.logStateChange('SIGTERM', 'Sinal SIGTERM recebido - aplicação sendo encerrada');
            console.log('🚨 [SIGNAL] SIGTERM recebido');
        });
        
        process.on('SIGINT', () => {
            this.logStateChange('SIGINT', 'Sinal SIGINT recebido - aplicação sendo encerrada');
            console.log('🚨 [SIGNAL] SIGINT recebido');
        });
        
        // Capturar saída do processo
        process.on('exit', (code) => {
            this.logStateChange('exit', `Processo sendo encerrado com código: ${code}`);
            console.log(`🚨 [EXIT] Processo encerrando com código: ${code}`);
        });
        
        console.log('🛡️ [SETUP] Handlers globais de erro configurados');
    }
    
    // Função para logar mudanças de estado
    logStateChange(action, description, stackTrace = null) {
        try {
            console.log(`🔍 [STATE-LOG] ${action}: ${description}`);
        } catch (error) {
            console.error('❌ Erro ao logar mudança de estado:', error);
        }
    }
    ...
    saveState() {
        try {
            // Log quando isActive muda
            const previousState = this.loadState();
            if (previousState && previousState.isActive !== this.isRunning) {
                this.logStateChange('saveState', `isActive alterado de ${previousState.isActive} para ${this.isRunning}`);
            }
            
            // Coletar informações detalhadas sobre conexões
            const caixasStatus = {};
            for (const [caixaId, caixaData] of this.caixaConnections) {
                caixasStatus[caixaId] = {
                    isConnected: caixaData.isConnected,
                    lastConnectionAttempt: caixaData.lastConnectionAttempt?.toISOString(),
                    connectionRetries: caixaData.connectionRetries || 0,
                    caixaName: caixaData.config.nome
                };
            }
            ...
            const state = {
                ...
                externalApiConfig: this.globalConfig || {},
                connectedCaixas: Array.from(this.caixaConnections.keys()),
                caixasStatus: caixasStatus,
                ...
                intervals: {
                    ...
                    reconnectionInterval: this.reconnectionInterval ? true : false
                },
    ...
    clearState(reason = 'Não especificado', authorized = false) {
        try {
            // Log detalhado sobre quem está chamando clearState
            const stack = new Error().stack;
            const timestamp = new Date().toISOString();
            
            // Verificar se a chamada é autorizada
            if (!authorized) {
                this.sendLog('🚨 ALERTA CRÍTICO: clearState() chamado SEM AUTORIZAÇÃO!');
                this.sendLog(`⚠️ Motivo: ${reason}`);
                this.sendLog(`🕐 Timestamp: ${timestamp}`);
                this.sendLog('📍 Stack trace completo:');
                console.error('🚨 UNAUTHORIZED CLEAR STATE DETECTED!');
                console.error('Reason:', reason);
                console.error('Timestamp:', timestamp);
                console.error('Stack:', stack);
                
                // Adicionar delay para investigação em produção
                console.error('⚠️ POSSÍVEL RESET NÃO AUTORIZADO DO SERVIDOR!');
            } else {
                this.sendLog(`🔄 clearState() autorizado - Motivo: ${reason}`);
                console.log(`✅ Authorized clearState - Reason: ${reason}`);
            }
            
            // Log adicional para arquivo específico
            this.logStateChange('clearState', `isActive alterado para FALSE - Motivo: ${reason} - Autorizado: ${authorized}`, stack);
            ...
            const emptyState = {
                ...
                lastClearState: timestamp,
                clearStateReason: `${reason} - Autorizado: ${authorized}`,
                clearStateStack: stack.split('\n').slice(0, 10).join('\n') // Primeiras 10 linhas do stack
            };
    ...
    async restoreFromState() {
        try {
            ...
            if (!state || !state.isActive || !state.caixasConfigs || state.caixasConfigs.length === 0) {
                ...
                if (state) {
                    this.sendLog(`📋 Estado encontrado: isActive=${state.isActive}, caixas=${state.caixasConfigs?.length || 0}`);
                }
                return false;
            }
            
            this.sendLog('🔄 Restaurando sincronização do estado salvo...');
            this.sendLog(`📊 Estado salvo em: ${state.lastStateUpdate || 'data não disponível'}`);
            this.sendLog(`🏪 Caixas configurados: ${state.caixasConfigs.length}`);
            
            // Mostrar status dos caixas se disponível
            if (state.caixasStatus) {
                for (const [caixaId, status] of Object.entries(state.caixasStatus)) {
                    const statusText = status.isConnected ? '✅ Conectado' : '❌ Desconectado';
                    this.sendLog(`🏪 ${status.caixaName}: ${statusText} (tentativas: ${status.connectionRetries})`);
                }
            }
            ...
            if (state.apiStats) {
                ...
                this.sendLog('📈 Estatísticas da API restauradas');
            }
            
            // Restaurar configurações da API externa se existirem
            if (state.externalApiConfig) {
                this.globalConfig = { ...this.globalConfig, ...state.externalApiConfig };
                this.sendLog('⚙️ Configurações da API externa restauradas');
            }
            
            // Restaurar token se existir
            if (state.token) {
                this.token = state.token;
                this.sendLog('🔑 Token da API restaurado');
            }
            ...
            this.sendLog(`🔍 Stack trace: ${error.stack}`);
            
            // Verificar se é um erro temporário de conexão
            const isTemporaryError = error.message.includes('ORA-') || 
            ...
            if (isTemporaryError) {
                this.sendLog('⚠️ Erro temporário detectado. Mantendo estado para nova tentativa de restauração.');
                this.sendLog('🔄 O sistema tentará restaurar novamente na próxima inicialização');
                // Não limpar o estado para erros temporários - permitir nova tentativa
                return false;
            } else {
                this.sendLog('❌ Erro crítico detectado. Limpando estado corrompido.');
                // Limpar estado apenas para erros críticos/permanentes
                this.clearState('Erro crítico na restauração do estado', false);
                return false;
            }
        }
    }
    ...
    updateExternalApiConfig(externalConfig) {
        if (externalConfig && externalConfig.externalApi) {
            ...
            // Log das mudanças
            this.logStateChange('updateExternalApiConfig', 
                `Configurações da API externa atualizadas: URL=${this.globalConfig.EXDAPIURL}, USER=${this.globalConfig.EXDUSERAPI}`);
            
            // Invalidar token para forçar novo login com novas credenciais
            this.token = null;
            
            // Salvar estado atualizado se a sincronização estiver ativa
            if (this.isRunning) {
                this.saveState();
                this.sendLog('💾 Estado atualizado com novas configurações da API externa');
            }
    ...
    async connectToCaixa(caixaConfig) {
        try {
            ...
            // Registrar tentativa de conexão falhada
            const existingData = this.caixaConnections.get(caixaConfig.id);
            if (existingData) {
                existingData.isConnected = false;
                existingData.lastConnectionAttempt = new Date();
                existingData.connectionRetries = (existingData.connectionRetries || 0) + 1;
            } else {
                this.caixaConnections.set(caixaConfig.id, {
                    connection: null,
                    config: caixaConfig,
    ...
    async executeQueryForCaixa(caixaId, query, params = {}, options = {}) {
        ...
        try {
            ...
            // Marcar conexão como perdida se for erro de conexão
            if (error.message.includes('ORA-') || error.message.includes('connection')) {
                caixaData.isConnected = false;
                this.sendLog(`⚠️ Conexão perdida com caixa ${caixaData.config.nome}`, caixaData.config.nome);
            }
    ...
    // Verificar e reconectar caixas desconectados
    async checkAndReconnectCaixas() {
        if (!this.isRunning) {
            return;
        }
        
        this.sendLog('🔍 Verificando status das conexões dos caixas...');
        
        for (const [caixaId, caixaData] of this.caixaConnections) {
            if (!caixaData.isConnected) {
                const timeSinceLastAttempt = Date.now() - (caixaData.lastConnectionAttempt?.getTime() || 0);
                const minRetryInterval = Math.min(30000 * Math.pow(2, caixaData.connectionRetries || 0), 300000); // Backoff exponencial, máximo 5 minutos
                
                if (timeSinceLastAttempt >= minRetryInterval) {
                    this.sendLog(`🔄 Tentando reconectar ao caixa ${caixaData.config.nome}...`, caixaData.config.nome);
                    await this.connectToCaixa(caixaData.config);
                }
            } else if (caixaData.connection) {
                // Testar se a conexão ainda está ativa
                try {
                    await caixaData.connection.execute('SELECT 1 FROM DUAL');
                    // Resetar contador de tentativas se conexão está OK
                    caixaData.connectionRetries = 0;
                } catch (error) {
                    this.sendLog(`⚠️ Conexão perdida detectada para caixa ${caixaData.config.nome}: ${error.message}`, caixaData.config.nome);
                    caixaData.isConnected = false;
                    caixaData.lastConnectionAttempt = new Date();
                    
                    // Tentar fechar conexão antiga
                    try {
                        await caixaData.connection.close();
                    } catch (closeError) {
                        // Ignorar erros ao fechar conexão inativa
                    }
                    caixaData.connection = null;
                }
            }
        }
    }
    ...
    async startSync(caixasConfigs, globalConfig = {}) {
        ...
        // Log detalhado sobre quem está chamando startSync
        const stack = new Error().stack;
        this.sendLog('🚀 ALERTA: startSync() foi chamado - sincronização será iniciada');
        this.sendLog('📍 Stack trace da chamada:');
        console.log(stack);
        
        // Log adicional para arquivo específico
        this.logStateChange('startSync', 'Sincronização sendo iniciada - isActive será alterado para TRUE', stack);
        ...
        // Removido: validação que impedia iniciar sincronização sem caixas conectados
        // A sincronização deve continuar mesmo sem conexões iniciais, pois há reconexão automática
        
        this.sendLog(`${connectedCount} de ${caixasConfigs.length} caixas conectados com sucesso`);
        if (connectedCount === 0) {
            this.sendLog('⚠️ Nenhum caixa conectado inicialmente, mas sincronização continuará com reconexão automática');
        }
        ...
        // Configurar intervalo de reconexão automática (verificar a cada 2 minutos)
        this.reconnectionInterval = setInterval(() => {
            this.checkAndReconnectCaixas();
        }, 2 * 60 * 1000);
    ...
    async stopSync() {
        ...
        // Log detalhado sobre quem está chamando stopSync
        const stack = new Error().stack;
        this.sendLog('🚨 ALERTA: stopSync() foi chamado - sincronização será parada');
        this.sendLog('📍 Stack trace da chamada:');
        console.log(stack);
        
        // Log adicional para arquivo específico
        this.logStateChange('stopSync', 'Sincronização sendo parada - isActive será alterado para FALSE', stack);
        ...
        // Limpar estado da sincronização
        this.clearState('Parada manual da sincronização via stopSync()', true);
        
        // Parar monitor do sistema se necessário
